...
public interface TradesRepository extends JpaRepository<TradesModel, Long> {
    ...
    @Query("SELECT new com.market_mind.market_mind_web.dto.PriceAndVolumeDTO(AVG(t.price) as price, SUM(t.volume) as volume, MIN(t.tradeTimestamp) as tradeTimestamp) FROM TradesModel t WHERE t.tradeTimestamp BETWEEN :startDate AND :endDate GROUP BY FUNCTION('HOUR', t.tradeTimestamp), FUNCTION('MINUTE', t.tradeTimestamp), FUNCTION('SECOND', t.tradeTimestamp)")
    ...
    @Query("SELECT new com.market_mind.market_mind_web.dto.PriceAndVolumeDTO(AVG(t.price) as price, SUM(t.volume) as volume, MIN(t.tradeTimestamp) as tradeTimestamp) FROM TradesModel t WHERE t.tradeTimestamp BETWEEN :startDate AND :endDate GROUP BY FUNCTION('HOUR', t.tradeTimestamp), FUNCTION('MINUTE', t.tradeTimestamp)")
